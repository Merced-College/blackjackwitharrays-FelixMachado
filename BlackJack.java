   ...
   public static void main(String[] args) {
       ...
       while (playAgain) {
           ...
           if (playerTotal > 21) { // if the player total is greater than 21 do the following command
               ...
               dealerWins++; // Increment dealer win counter
           } else {
               dealerTotal = dealerTurn(dealerTotal);
               if (determineWinner(playerTotal, dealerTotal)) {
                   playerWins++; // Increment player win counter
               } else {
                   dealerWins++; // Increment dealer win counter
               }
           }
           ...
           if (playAgain) {
               currentCardIndex = 0; // Reset card index for a new game
           }
       }
...
// While LOOP LOCATED ABOVE HERE /\
   ...
   // This approach shows the player's decision-making during their turn, making
   // sure that their input works and goes through while managing the outcomes of
   // their actions
   private static int playerTurn(Scanner scanner, int playerTotal) {
       while (true) {
           ...
           String action = scanner.nextLine().toLowerCase(); // .toLowerCase (makes whatever the player types
                                                             // lowercase)
           if (action.equals("hit")) {
               ...
               System.out.println(
                       "You drew a " + RANKS[newCard] + " of " + SUITS[DECK[currentCardIndex] % 4]);
               if (playerTotal > 21) {
                   break;
               }
   ...
   // This makes the dealer continue to deal themselves a card until dealerTotal <
   // 17 is false
   ...
   // Determines if the player wins vs Dealer through comparing the dealerTotal
   // with 21 and playerTotal
   ...
   // Deals Cards to the players the currentCardIndex++ keeps track of the position
   // of the next card to be dealt
   ...
   // linearSearch checks each element of the array for a match to the key and
